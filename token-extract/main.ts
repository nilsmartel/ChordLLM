...
function getCliArgs(): [string, string] {
    ...
    if (filepath == null || filepath == '--help') {
        console.log(
            'expected first argument to be filepath or - for stdin. Expected second argument to be list of almost any allowed tokens'
        )
        process.exit(1)
    }
...
let filenames = []
try {
    filenames = isDirectory(filename)
        ? fs.readdirSync(filename).map((f) => path.join(filename, f))
        : [filename]
} catch (e) {
